...
# Main
# --------------------------------------------------------------------------
def run_pipeline_flattened(xyz_file, skip_header=None, n_slices=200, profile_bins=300,
    ...
    # Coarse global flatten, done up front and used for ALL detection
    # (peak location, prominence threshold, and FWHM). Fit a robust
    # surface to the whole cloud -- iterative outlier rejection so
    # ridge points naturally get down-weighted out of the fit without
    # needing to already know where the ring is -- then subtract it from
    # everything before any slicing happens.
    coarse_coeffs, _ = fit_surface(pts[:, 0], pts[:, 1], pts[:, 2],
                                    order=surface_order, robust_iters=2)
    pts_detect = pts.copy()
    pts_detect[:, 2] = pts[:, 2] - evaluate_surface(
        coarse_coeffs, pts[:, 0], pts[:, 1], surface_order
    )
    z = pts_detect[:, 2]
    ...
    if prominence is None:
        ...
        sigma = 1.4826 * mad
        # Use ~5 sigma
        prominence = 5 * sigma if sigma > 0 else 0.1 * (z.max() - z.min())
        print(f"Auto prominence threshold (from coarsely-flattened data, "
              f"noise sigma~{sigma:.4g}): {prominence:.4g}")

    
    thresh = np.percentile(z, z_percentile)
    hi_pts = pts_detect[z >= thresh]
    ...
    cand_xz = slice_scan(pts_detect, slice_axis=1, cross_axis=0, other_axis=1,
    ...
    cand_yz = slice_scan(pts_detect, slice_axis=0, cross_axis=1, other_axis=0,
                          n_slices=n_slices, profile_bins=profile_bins,
                          prominence=prominence,
                          min_peak_distance=min_peak_distance,
                          min_pts=min_pts)
    ...
    if not no_flatten:
        if "outer" not in ellipses:
            ...
            # This final flatten is fit fresh from the RAW cloud (not the
            # coarse pre-detection flatten), using the now-known outer
            # ellipse to exclude the ring and the cut interior from the
            # baseline fit -- a more targeted, accurate correction than
            # the coarse whole-cloud fit used earlier just to make
            # detection robust to tilt.
            ...
            # Re-express each edge group's Z from "height above the coarse
            # pre-detection surface" to "height above this final surface".
            # group[:, 2] is NOT raw Z -- it's already coarse-flattened,
            # since detection ran on pts_detect. Naively subtracting the
            # final surface again would double-flatten it. Instead:
            #   raw_z         = coarse_z + coarse_surface(x, y)
            #   final_flat_z  = raw_z - final_surface(x, y)
            #                 = coarse_z + coarse_surface(x, y) - final_surface(x, y)
            for name in ("inner", "outer"):
                group = results[name]["points"]
                if len(group) == 0:
                    continue
                coarse_surf_z = evaluate_surface(coarse_coeffs, group[:, 0], group[:, 1],
                                                  surface_order)
                final_surf_z = evaluate_surface(surf_coeffs, group[:, 0], group[:, 1],
                                                 surface_order)
                group_flat = group.copy()
                group_flat[:, 2] = group[:, 2] + coarse_surf_z - final_surf_z
    ...
    for name in ("inner", "outer"):
        ...
        print(f"\n--- {name.upper()} EDGE ---")
        print(f"  Z stats{' (final flattened)' if flat_pts is not None else ' (coarse flattened)'}: "
